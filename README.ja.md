...
- **キーボードショートカット**: 入力欄（`<input>` / `<textarea>` / `contenteditable` / `role="textbox"` / `role="searchbox"`）フォーカス中は自動で無効化
...
- **多言語対応（UI）**: ブラウザ UI 言語に応じてポップアップと拡張機能名を英語 / 日本語に切り替え（`chrome.i18n` を使用、デフォルトは英語）
...
- **ショートカットキー再割当**: 各キーボタンをクリック → 新しいキーを押下 → 保存
  - 受け付けるのは **A〜Z と 0〜9 のみ**。修飾キー（Ctrl / Alt / Shift / Meta）付きや記号は拒否される
  - 既に他のアクションに割り当て済みのキーは拒否される
  - `Esc` 押下またはボタンからフォーカスを外すとキャンセル
- **Reset to defaults**: 3 つのショートカットを一括で既定値（`S` / `D` / `R`）に戻す
- **現在のサイト**: このドメインを除外リストに追加/解除（変更は開いているページにもすぐに反映）
...
`R` キーは再生速度を `1.0x` と内部的な **指定速度（target、トグルの切替先）** の間でトグルします。target はポップアップから直接設定できる項目ではなく、Up / Down で調整した速度に自動追従します。具体的には、Up / Down で `current` が変わるたびに `target` も **同じ値に上書き** されます。

唯一の例外は `0.9` 〜 `1.1` の範囲（両端含む）です。この範囲でのトグルは体感しにくいため、`target` がこの範囲に入ることは許されません。Up / Down で新しい current 値がこの範囲に入るときは、**target の書き込みをスキップ**して据え置きます。

### 例

| 初期 (current, target) | 操作 | 新 current | 新 target |
| ---- | ---- | ---- | ---- |
...
```
MarkNVideoSpeed/
├── manifest.json                        Manifest V3（name / description は __MSG_*__ で i18n 化）
├── src/
...
│   └── popup/                           ポップアップ UI（chrome.i18n.getMessage で文字列を差し替え）
│       ├── popup.html
│       ├── popup.css
│       └── popup.js
├── _locales/                            UI メッセージカタログ（Chrome i18n）
...
設計・実装方針の参考として [igrigorik/videospeed](https://github.com/igrigorik/videospeed) を調査しました。
